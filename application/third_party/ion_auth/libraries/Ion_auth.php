...
class Ion_auth
{
	...
	public function admission_register($identity, $password, $email, $additional_data = array(), $group_ids = array())
	{
		$this->ion_auth_model->trigger_events('pre_account_creation');

		$email_activation = $this->config->item('email_activation', 'ion_auth');

		$id = $this->ion_auth_model->register($identity, null, $email, $additional_data, $group_ids);

		if($id && $this->input->post('courseId') && $this->input->post('courseType')){
			$extradata = [
				'courseid' => $this->input->post('courseId'),
				'courseType' => $this->input->post('courseType'),
				'user_id'	=> $id
			];
			$this->ion_auth_model->insertUserCourseEnroll($extradata);
		}
		if (!$email_activation)
		{
			if ($id !== FALSE)
