...
<!--  -->        
        <section  class="panel" id="advanced_search">
            <div class="panel-body">
                <h4 class=""> <strong>Blocks List</strong>
                <a href="<?php echo base_url('admin/block/add')?>" class="btn btn-primary fa fa-user-plus pull-right" > Add New Block</a></h4>
                <hr style="margin:5px0px;" />
                <form id="user_search">
                    <div class="col-md-12">
                        <label>Blocks State</label>
                        <hr style="margin:5px 0px;" />
                        <input checked="checked" onchange="user_filter(this)" name="user_search_type" id="active" value="active" type="radio"  /><label for="active">&nbsp;Active Blocks&nbsp;&nbsp;&nbsp;</label>

                        <input onchange="user_filter(this)" name="user_search_type" id="deactive" value="deactive" type="radio"  /><label for="deactive">&nbsp;Deactive Blocks&nbsp;&nbsp;&nbsp;</label>  

                        <input onchange="user_filter(this)" name="user_search_type" id="all" value="all" type="radio"  /><label for="all">&nbsp;ALL Blocks&nbsp;&nbsp;&nbsp;</label>  
                    </div>
                </form>
            </div>
        </section>
...
<script>
	...
	var table =	$('.mv_datatable').DataTable({
		...
		"serverSide": true,
		"fixedHeader": true,
		"ajax": "<?=base_url('admin/block/datatable_json')?>",
        "bPaginate": true,
		"order": [[0,'desc']],
		"columnDefs": [
						...
						{ "targets": 3, "name": "excerpt", 'searchable':true, 'orderable':true},
...
{ 
    $.post('<?=base_url('admin/block/search')?>',$('#user_search').serialize(),function(){
        table.ajax.reload( null, false );
    });
}
