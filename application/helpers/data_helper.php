...
function get_data($str,$file = false,$isFilter = true){// second para for switch
	$CI = &get_instance();
			// return (set_value($str))?set_value($str,'',):((isset($CI->data[$str]))?$CI->data[$str]:null);
        $defaultData = ((isset($CI->data[$str]))?$CI->data[$str]:null);
        return set_value($str,$defaultData,false);
}

function get_data2($str,$file = false,$isFilter = true){// second para for switch
	$CI = &get_instance();
	if(!$file){
...
// list of column key and their name for history
	...
	function parser_words($str,$data = null){
		if(empty($data)){
			$data = config_item('parser_words');
		}
